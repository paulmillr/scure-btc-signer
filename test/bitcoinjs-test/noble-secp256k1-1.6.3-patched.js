...
function initSchnorrSigArgs(message, privateKey, auxRand) {
  ...
  const rand = auxRand !== null ? ensureBytes(auxRand) : null;
  if (rand && rand.length !== 32) throw new TypeError('sign: Expected 32 bytes of aux randomness');
...
async function schnorrSign(message, privateKey, auxRand = utils.randomBytes()) {
  ...
  const t =
    rand === null ? numTo32b(d) : initSchnorrNonce(d, await utils.taggedHash(TAGS.aux, rand));
...
function schnorrSignSync(message, privateKey, auxRand = utils.randomBytes()) {
  const { m, px, d, rand } = initSchnorrSigArgs(message, privateKey, auxRand);
  const t = rand === null ? numTo32b(d) : initSchnorrNonce(d, utils.taggedHashSync(TAGS.aux, rand));
